...
export interface ColorModalProps {
  ...
  onSave: (color: { hex: string; name: string }) => void;
  userId: string;
  ...
  useEffect(() => {
    if (isOpen) {
      if (mode === "edit" && initialColor) {
        setColorValue(initialColor.hex);
    ...
    <div className="space-y-5 relative">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          <h4 className="font-semibold leading-none text-lg bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text text-transparent">
  ...
  const modalContent = (
    <>
      ...
      >
        ...
        {!isPositioned && (
          <div
            className="bg-gradient-to-br from-card via-card to-card/95 border-2 border-primary/20 p-6 w-80 max-w-[90vw] shadow-2xl relative overflow-hidden animate-in fade-in zoom-in-95 duration-200 rounded-lg"
      ...
      {isPositioned && (
        <div
          className="fixed bg-gradient-to-br from-card via-card to-card/95 border-2 border-primary/20 w-80 max-w-[90vw] shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 z-[10000] rounded-lg"
          style={{
            top: `${position.top}px`,
